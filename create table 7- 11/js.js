  ...
  for (let i = 0 ; i < people.length; i++) {
      ...
      for (let j = 0; j < arrWithSectionNames.length; j++) {
          ...
          switch (arrWithSectionNames[j]) {
            ...
            td.setAttribute('data-selectable', 'age');
            tr.appendChild(td);
            break;
            case 'century': 
...
function addBorder( age) {
  [...document.querySelectorAll('.person')].map( el => {
    if( +el.children[5].textContent > +age ) {
      [...el.children].map(section => {
        section.classList.add('green-border');
      })
    }
...
const getPeopleHTML = () => {
  return tableWithPeople.innerHTML;
}
...
tableWithPeople.addEventListener( 'click' , (event) => {
  let target = event.target;
  let peoplesInTable = [...document.querySelectorAll('.person')];
  let tbody = document.querySelector('tbody');
  if(target.textContent === 'name') {
    tbody.innerHTML = null;
    peoplesInTable.sort((a,b) => {
      if(a.children[1].innerText < b.children[1].innerText) { return -1};
      if(a.children[1].innerText > b.children[1].innerText) { return 1};
      return 0;
    });

    for (let i = 0; i < peoplesInTable.length; i++) {
      tbody.appendChild(peoplesInTable[i]);
    };
  };
  if (target.textContent === 'age') {
    tbody.innerHTML = null;
    peoplesInTable.sort((a,b)=> {
      if(a.children[4].innerText - a.children[3].innerText <
         b.children[4].innerText - b.children[3].innerText) { return -1};
      if(a.children[4].innerText - a.children[3].innerText > 
        b.children[4].innerText - b.children[3].innerText) { return 1};
      return 0;
    });
  ...
  if (target.textContent === 'died') {
    tbody.innerHTML = null;
    peoplesInTable.sort((a,b)=> {
      if(a.children[4].innerText < b.children[4].innerText) { return -1};
      if(a.children[4].innerText > b.children[4].innerText) { return 1};
      return 0;
    });
    for (let i = 0; i < peoplesInTable.length; i++) {
  ...
  if (target.textContent === 'born') {
    tbody.innerHTML = null;
    peoplesInTable.sort((a,b)=> {
      if(a.children[3].innerText < b.children[3].innerText) { return -1}
      if(a.children[3].innerText > b.children[3].innerText) { return 1}
      return 0;
    })
    for (let i = 0; i < peoplesInTable.length; i++) {
  ...
  if (target.textContent === 'id') {
    document.querySelector('.people').removeChild(document.querySelector('.people__table'));
    showPeople(document.querySelector('.people'), people);
    addAllStyles();
  }
});
...
tableWithPeople.addEventListener( 'click' , (event) => {
  ...
  if(isData(target)) {
    if(selectedTD) {
      selectedTD.classList.remove('blue-border');
    }
    selectedTD = target;
    selectedTD.classList.add('blue-border');
  } else {
    return;
  }
...
let peoplesInTable = [...document.querySelectorAll('.person')];
let textAboveInput = document.createElement('h2');
  textAboveInput.textContent = 'Please type a Full Name and Press ENTER/))))';
  document.body.appendChild(textAboveInput);
  document.body.appendChild(input);

input.addEventListener('keypress', el => {
  let valueOfUser = input.value;
  if(el.keyCode === 13) {
    if(valueOfUser !== '') {
      let idOfPeople = [];
      let findedPeople = [];
      let searchPeople =  peoplesInTable.filter( el => {
        valueOfUser.trim();
        if(el.children[1].innerText === valueOfUser ||
           el.children[7].innerText === valueOfUser ||
           el.children[8].innerText === valueOfUser) {
          idOfPeople.push(el.children[0].innerText);
          return true;
        }
       });
       if(searchPeople.length !== 0) {
         for (let i = 0; i < peoplesInTable.length; i++) {
           let id = peoplesInTable[i].children[0].innerText
           for (let j = 0; j < idOfPeople.length; j++) {
              if( id === idOfPeople[j]) {
                findedPeople.push(peoplesInTable[i]);
              }
           }
         }
         let tbody = document.querySelector('tbody');
         ...
         for (let i = 0; i < findedPeople.length; i++) {
           tbody.appendChild(findedPeople[i]);
         }
       }else {
        input.value = '';
         alert('No such people . Try again');
       }
    }
...
//task 11
  function isData(el) {
    if(el.dataset.selectable === 'age' ||
       el.dataset.selectable === 'name' ||
       el.dataset.selectable === 'century') {
      return true;
    }
    return false;
  }
